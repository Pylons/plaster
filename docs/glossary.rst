...
.. glossary::
    ...
    config uri
        In most cases this is simply an absolute or relative path to a config
        file on the system. However, it can also be a :rfc:`1738`-style string
        pointing at a remote service or a specific parser without relying on
        the file extension. For example, ``my-ini://foo.ini`` may point to a
        loader named ``my-ini`` that can parse the relative ``foo.ini`` file.
